...
// import { publicApi } from '@/lib/api';   // ⬅️ assumes GET /categories/slug/{slug}
//                                           //     returns { id, name, … }
...
//     categoryId:  initialData?.categoryId || '',   // holds the **id** once resolved
...
//     const slug = e.target.value;            // option value is the slug
//     if (!slug) {
//       setFormData((p) => ({ ...p, category: '' }));
//       return;
//     }
//     try {
//       const { data } = await publicApi.get(`/categories/slug/${slug}`);
//       setFormData((p) => ({ ...p, category: data.id })); // store the id
//     } catch (err) {
...
//         categoryId: formData.category,   // already resolved to id
...
//   /* Quill config left unchanged ↓ */
//   const modules = { toolbar: [[{ header:[1,2,3,4,5,6,false] }], ['bold','italic','underline','strike'],
//                               [{ color:[] },{ background:[] }],[{ align:[] }],
//                               ['blockquote','code-block'],[{ list:'ordered' },{ list:'bullet' }],
//                               [{ indent:'-1' },{ indent:'+1' }],['link','image'],['clean']]};

//   const formats = ['header','bold','italic','underline','strike','color','background','align',
...
//         <input id="title" name="title" required value={formData.title} onChange={handleInput}
...
//         <textarea id="excerpt" name="excerpt" rows="3" value={formData.excerpt} onChange={handleInput}
//                   className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500"
...
//           <ReactQuill value={formData.content} onChange={handleContent}
...
//                checked={formData.published} onChange={handleInput}
